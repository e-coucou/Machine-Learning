...
import torch
import argparse
import tools.ai_token as tk  # Ton fichier tokenizer.py
import tools.ai_gpt_ep as gpt  # Ton fichier où se trouve GenerateGPT
...
def main():
    ...
    parser.add_argument("prompt", type=str, 
                        help="Le texte de départ (amorce) pour l'IA.")
    ...
    parser.add_argument("-n", "--tokens", type=int, default=100, 
                        help="Nombre maximum de nouveaux tokens à générer (défaut: 100).")
    
    parser.add_argument("-k", "--topk", type=int, default=40, 
                        help="Limite le choix aux K mots les plus probables (défaut: 40).")
    
    parser.add_argument("-p", "--rep_penalty", type=float, default=1.2, 
                        help="Pénalité de répétition (défaut: 1.2).\n> 1.0 réduit les boucles infinies (ex: 'de l'ADN de l'ADN').")
    
    parser.add_argument("-m", "--model", type=str, default="model/my_wiki_inference.pth", 
                        help="Chemin vers le fichier .pth du modèle (défaut: model/my_wiki_inference.pth).")
