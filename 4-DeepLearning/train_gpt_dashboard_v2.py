import json, re, time, os, psutil, subprocess, math
import numpy as np
import torch, gc
...
# Raccourci module-level (rétro-compatibilité des f-strings)
UI        = Config.UI
LIGNE_LEN = Config.LIGNE_LEN
...
#        mps = torch.mps.current_allocated_memory() / 1_048_576  # en Mo
        ...
        status = ("🟣" if pression < 70 else
                  "🟢" if pression < 80 else
                  "🟡" if pression < 85 else
                  "🟠" if pression < 90 else "\U0001F534")
...
#                print(line.strip())
            ...
            if len(last_line) >= 3:
                ...
                tps          = float(last_line[1])  # Transfert Par Seconde
...
# =============================================================================
#  MODÈLES DE LOSS ET PRÉDICTIONS
# =============================================================================
class LossForecaster:
    ...
    def get_forecast_old(steps, losses, horizons):
        try:
            ...
            popt, _ = curve_fit(LossForecaster.loss_model, x_data, y_data, p0=p0, maxfev=2000)

            predictions = {}
            for h in horizons:
                future_step    = steps[-1] + h
                pred           = LossForecaster.loss_model(future_step, *popt)
    ...
    def get_forecast(steps, losses, horizons):
        try:
            ...
            popt, _ = curve_fit(LossForecaster.loss_model, x_data, y_data, p0=p0, bounds=bounds, maxfev=5000)
...
class Analyzer:
    ...
    def get_intel_level(ema_loss):
        ...
        if ema_loss > 2.50: return "ÉTUDIANT. ", "🔵",  "\033[94m"           # 11
        if ema_loss > 2.35: return "EXPERT.   ", "🟣",  "\033[95m"           # 10
        return                     "MÉMORISATION","🔥", "\033[1;38;5;201m"

    @staticmethod
    def get_speed_level(tok_s):  # ce sont désormais des tokens/s
        if tok_s < 800:    return "🔴", UI["RED"],     "SURCHAUFFE"
        elif tok_s < 850:  return "🟠", UI["ORANGE"],  "BOUILLANT"
        elif tok_s < 885:  return "🟡", UI["YELLOW"],  "CHAUD"
        elif tok_s < 910:  return "🟢", UI["GREEN"],   "FROID"
        elif tok_s < 930: return "🔵", UI["BLUE"],    "GLACE"
        else:              return "🟣", UI["MAGENTA"], "-----"

    @staticmethod
    def get_green_level(level):
        # Calcul de l'indicateur visuel
        if level < 5:    return "🌱"   # Exceptionnel (ton cas actuel)
        elif level < 8:  return "🍀"   # Très bon
        elif level < 12: return "🌿"   # Très bon
        elif level < 15: return "🍂"   # Le swap ou la chauffe ralentit tout
        else:            return "☢️"   # c'est la panique ...

    @staticmethod
    def get_bias_status(prc):
#       bias_wiki = f"{UI['RED']}\u25bc Spéc." if bias_wiki_v<-5 else f"{UI['RED']}\u25b2 Galère" if bias_wiki_v >5 else f"{w}\u25ac Optimal"
        prc_abs     = abs(prc)
        bias_status = (f"{UI['RED']}\u25bc Overfit"      if prc < -15. else
                       f"{UI['RED']}\u25b2 Galère"        if prc > 15   else
                       f"{UI['MAGENTA']}\u25ac Optimal"   if prc_abs < 5 else
                       f"{UI['BLUE']}\u25ac Div. No."     if prc < 15   else
                       f"{UI['ORANGE']}\u25ac Alerte")
        return bias_status
    ...
    def get_gn_status(norms):
        ...
        return status, avg_gn, max_gn, min_gn
...
class CheckpointLoader:
    ...
    def get_checkpoint(self, path):
        ...
        if self.first:
            self.model_ema = ckpt.get('model_ema', None)
            self.n_params  = sum(t.numel() for t in ckpt['model'].values() if isinstance(t, torch.Tensor))
            self.first     = False
        del ckpt
...
class Dashboard:
    ...
    def __init__(self):
        self.update            = {'status': 0, 'step': 0, 'elapse': 244}
...
#       steps, losses, times, lr, trains = parse_logs(LOG_FILE)
        last_update = os.path.getmtime(Config.HISTORY_FILE)
        steps_h, trains_h, vals_h, times_h, vals_h_wiki, vals_h_cult, vals_h_litt = DataParser.parse_history(Config.HISTORY_FILE)

        gap_h        = vals_h - trains_h
        data_monitor = DataParser.parse_monitor(Config.MONITOR_FILE)

        file                                = Config.MODEL_FILE
        status_RAM, pression, mem_rss, swap = SystemMonitor.get_memory_status()
        step, wiki, cult, litt, config, params = self.checkpoint_loader.get_checkpoint(file)
        batch_size       = params.get('batch_size', 32)
        block_size       = config.get('block_size', Config.BLOCK_SIZE)
        target_train     = params.get('lr_decay_iters', Config.TARGET_TRAIN)
...
#       x = np.arange(20)
        ...
        _, _, wiki_status = Analyzer.get_intel_level(vals_h_wiki[-1])
        _, _, cult_status = Analyzer.get_intel_level(vals_h_cult[-1])
        _, _, litt_status = Analyzer.get_intel_level(vals_h_litt[-1])
        ...
        bias_wiki   = Analyzer.get_bias_status(bias_wiki_v)
        bias_cult_v = (vals_h_cult[-1] / vals_h[-1] - 1) * 100.
        bias_cult   = Analyzer.get_bias_status(bias_cult_v)
        bias_litt_v = (vals_h_litt[-1] / vals_h[-1] - 1) * 100.
        bias_litt   = Analyzer.get_bias_status(bias_litt_v)
        ...
        bar_train = "▬" * (round(pct_train/4)) + f"{UI['RED']}\u2719" + UI['DIM'] + UI['WHITE'] + "┅" * (25 - round(pct_train/4))  # "▄"
#        bar_train = "▬" * (round(pct_train/4)-1) + f"{UI['RED']}\u2719" + UI['DIM'] + UI['WHITE'] + "┅" * (25 + (-1 if round(pct_train/4)==0 else 0) - round(pct_train/4))  # "▄"

        # Alerte Thermique (basée sur le temps de cycle)
        # Si le Mac met plus de 195s pour 25 steps, on affiche en Orange/Rouge
        # temp_icon = "🌡️ "
        temp_icon, temp_color, temp_text = Analyzer.get_speed_level(tok_s)  # on passe en token par seconde
        ...
        ssd_usage,  ssd_status,  ssd_color              = SystemMonitor.get_ssd_usage()
        cpu_usage,  cpu_status,  cpu_color               = SystemMonitor.get_cpu_usage()
        gpu_usage,  gpu_status,  gpu_color, gpu_watts    = SystemMonitor.get_gpu_usage()
        throttle_info, throttle_status, throttle_color   = SystemMonitor.get_throttle()
        ...
        green_status  = Analyzer.get_green_level(mJ_tok)

        lr_h = []
        for it in data_monitor:  # steps_h:
            lr_h.append(Utils.calcul_lr(it['step'], params))
        lr_h = np.array(lr_h)

        lr_status = (f"{UI['MAGENTA']}\u25bc" if lr_h[-1] < lr_h[-2] else
                     f"{UI['MAGENTA']}\u25b2" if lr_h[-1] > lr_h[-2] else
                     f"{w}\u25ac")
        ...
        # Progression de l'entrainement epoch et steps total
        epoch_1  = int((data_monitor[-1]['dataset1'] * batch_size / Config.TARGET_BLOCK_DS1))
        pct_ds1  = (data_monitor[-1]['dataset1'] * batch_size / Config.TARGET_BLOCK_DS1 - epoch_1) * 100
        epoch_2  = int((data_monitor[-1]['dataset2'] * batch_size / Config.TARGET_BLOCK_DS2))
        pct_ds2  = (data_monitor[-1]['dataset2'] * batch_size / Config.TARGET_BLOCK_DS2 - epoch_2) * 100
        epoch_3  = int((data_monitor[-1]['dataset3'] * batch_size / Config.TARGET_BLOCK_DS3))
        pct_ds3  = (data_monitor[-1]['dataset3'] * batch_size / Config.TARGET_BLOCK_DS3 - epoch_3) * 100
        pct      = (steps_h[-1] / Config.TARGET_STEP) * 100
        epoch    = f"[epoch {int(pct/100 + 1)}]"
        if pct > 100:
            pct = pct % 100
        bar_ds1 = "▬" * (round(pct_ds1/4)) + f"{UI['RED']}\u2719" + UI['DIM'] + UI['CYAN'] + "┅" * (25 - round(pct_ds1/4))
#        bar_ds1 = "▬" * (round(pct_ds1/4)) + f"{UI['RED']}\u2719" + UI['DIM'] + UI['CYAN'] + "┅" * (25 + (-1 if round(pct_ds1/4)==0 else 0) - round(pct_ds1/4))
        bar_ds2 = "▬" * (round(pct_ds2/4)) + f"{UI['RED']}\u2719" + UI['DIM'] + UI['CYAN'] + "┅" * (25 - round(pct_ds2/4))
 #       bar_ds2 = "▬" * (round(pct_ds2/4)) + f"{UI['RED']}\u2719" + UI['DIM'] + UI['CYAN'] + "┅" * (25 + (-1 if round(pct_ds2/4)==0 else 0) - round(pct_ds2/4))
        bar_ds3 = "▬" * (round(pct_ds3/4)) + f"{UI['RED']}\u2719" + UI['DIM'] + UI['CYAN'] + "┅" * (25 - round(pct_ds3/4))
#        bar_ds3 = "▬" * (round(pct_ds3/4)) + f"{UI['RED']}\u2719" + UI['DIM'] + UI['CYAN'] + "┅" * (25 + (-1 if round(pct_ds3/4)==0 else 0) - round(pct_ds3/4))

        batch_ds1 = data_monitor[-1]['dataset1']
        batch_ds1_i = batch_ds1 - data_monitor[-15]['dataset1']
        batch_ds2 = data_monitor[-1]['dataset2']
        batch_ds2_i = batch_ds2 - data_monitor[-15]['dataset2']
        batch_ds3 = data_monitor[-1]['dataset3']
        batch_ds3_i = batch_ds3 - data_monitor[-15]['dataset3']
        s_ds = (batch_ds1_i + batch_ds2_i + batch_ds3_i)
        ratio_ds1 = (batch_ds1_i / s_ds)  # vs :  (1-params.get('cult_ratio',0.)-params.get('litt_ratio',0.))
        ratio_ds2 = (batch_ds2_i / s_ds)  # vs : (params.get('cult_ratio',0.))
        ratio_ds3 = (batch_ds3_i / s_ds)  # vs :  (params.get('litt_ratio',0.))
        n1 = int(data_monitor[-1]['dataset1'] * batch_size / Config.TARGET_BLOCK_DS1) + 1
        n2 = int(data_monitor[-1]['dataset2'] * batch_size / Config.TARGET_BLOCK_DS2) + 1
        n3 = int(data_monitor[-1]['dataset3'] * batch_size / Config.TARGET_BLOCK_DS3) + 1

        epoch_ds1, t = Utils.calcul_epoch(step=data_monitor[-1]['dataset1'], target=Config.TARGET_BLOCK_DS1, mix=ratio_ds1, batch=batch_size, grad=grad_accum_steps, t_step=data_monitor[-1]['step'], n=n1)
        epoch_ds2, _ = Utils.calcul_epoch(data_monitor[-1]['dataset2'], Config.TARGET_BLOCK_DS2, ratio_ds2, batch_size, grad_accum_steps, data_monitor[-1]['step'], n=n2)
        epoch_ds3, _ = Utils.calcul_epoch(data_monitor[-1]['dataset3'], Config.TARGET_BLOCK_DS3, ratio_ds3, batch_size, grad_accum_steps, data_monitor[-1]['step'], n=n3)

        gn_status, gn_avg, gn_max, gn_min = Analyzer.get_gn_status(data_monitor[-1]['grad_norm'])
        timeout_pattern = ['⠋', '⠙', '⠹', '⠸', '⠼', '⠴', '⠦', '⠧', '⠇', '⠏']

        if self.update['status'] == 1:
            self.update['status'] = 0
        else:
            if self.update['step'] != data_monitor[-1]['step']:
                self.update['step']   = data_monitor[-1]['step']
                self.update['status'] = 1

        # --- CALCUL DE LA LOSS LISSÉE (EMA) ---
        # On initialise l'EMA avec la première valeur de la liste
        ema_loss                          = Utils.calcul_ema(vals_h)
        ema_name, ema_icon, ema_color     = Analyzer.get_intel_level(ema_loss)
        loss_name, loss_icon, loss_color  = Analyzer.get_intel_level(vals_h[-1])
        _, _, train_color                 = Analyzer.get_intel_level(trains_h[-1])
        ...
        os.system('clear')
        Titre       = f"  🚀   M1 - LLM Training Monitor {Config.VERSION}.    - {steps_h[-1]} -"
...
#        header_right = f"{temp_color}{timeout_pattern[timeout]}  \033[1m{timestamp}\033[0m"
        ...
        padding = Config.LIGNE_LEN - len(Titre) - visible_right_len - 1
#        print(f"{UI['GRAY']}"+f"─" * LIGNE_LEN+f"{UI['RESET']}")
        ...
        efficiency, asymptote_new, trend_1k = Analyzer.calculate_saturation_score(steps_h, vals_h, lr_h, window=5000)
        status     = f"⚡ {UI['BLUE']}PRODUCTIF {UI['RESET']}" if efficiency > 0.5 else f"🐢 {UI['RED']}SATURATION{UI['RESET']}"
        level_name_new, _, color_asympt_new = Analyzer.get_intel_level(asymptote_new)
        ...
        graph     = Analyzer.get_micro_graph(gap_h[-15:], -1)

        # Affichage de l'entrainement en cours ...
        print(f"{UI['GRAY']}" + f"─" * Config.LIGNE_LEN + f"{UI['RESET']}")
...
#        print(f"  Epoch: {pct:4.1f}% {UI['CYAN']}┣{bar}┫ {UI['RESET']}pour {TARGET_STEP} steps")
        ...
        print(f"     Train:    {pct_train:5.2f}% {UI['CYAN']} ┣{bar_train}┫ {UI['RESET']}pour {Utils.f_num(target_train).rjust(10)} steps")
        print(f"     {w}Wiki ({epoch_1:1d}): {pct_ds1:5.2f}% {UI['CYAN']} ┣{bar_ds1}┫ {UI['RESET']}pour {Utils.f_num(Config.TARGET_BLOCK_DS1).rjust(10)} blocks")
        print(f"     {c}Cult ({epoch_2:1d}): {pct_ds2:5.2f}% {UI['CYAN']} ┣{bar_ds2}┫ {UI['RESET']}pour {Utils.f_num(Config.TARGET_BLOCK_DS2).rjust(10)} blocks")
        print(f"     {l}Litt ({epoch_3:1d}): {pct_ds3:5.2f}% {UI['CYAN']} ┣{bar_ds3}┫ {UI['RESET']}pour {Utils.f_num(Config.TARGET_BLOCK_DS3).rjust(10)} blocks")
        print(f"     Wikipédia  [{ratio_ds1*100:4.1f}] | batch: {batch_ds1:8} | end: {epoch_ds1:6} | {batch_ds1*batch_size*block_size*1e-6:6.1f}M tokens")
        print(f"     CulturaX   [{ratio_ds2*100:4.1f}] | batch: {batch_ds2:8} | end: {epoch_ds2:6} | {batch_ds2*batch_size*block_size*1e-6:6.1f}M tokens")
        print(f"     Litteraire [{ratio_ds3*100:4.1f}] | batch: {batch_ds3:8} | end: {epoch_ds3:6} | {batch_ds3*batch_size*block_size*1e-6:6.1f}M tokens")
        print(f"{UI['GRAY']}" + f"─" * Config.LIGNE_LEN + f"{UI['RESET']}")
...
#        print(f"     {w}Wikipédia/{c}CulturaX/{l}Litteraire : {w}{ratio_wiki:2.0f}% / {c}{ratio_cult:2.0f}% / {l}{ratio_litt:2.0f}% | {w}{bar}{UI['RESET']}")
        ...
        # Graphique amélioré du Log History
        mini  = vals_h[-50:]  # On prend plus de points pour le graph
        m_min, m_max = min(mini), max(mini)
        print(f"  📉 LOSS History |50| ", end="")
        # On utilise des caractères de blocs pour simuler une courbe
        levels = [" ", "_", "▂", "▃", "▄", "▅", "▆", "▇"]
        for v in mini:
            _, _, _color = Analyzer.get_intel_level(v)
            idx    = int(((v - m_min) / (m_max - m_min + 1e-6)) * 7)
            print(f"{_color}{levels[7 - idx]}", end="")
        print(f"\n{UI['GRAY']}" + f"─" * Config.LIGNE_LEN + f"{UI['RESET']}")
...
#        preds, asymptote = get_forecast(steps_h, vals_h, horizons)
        ...
        # Nouvelle Statistique de Prédiction ...
        horizon = target_train
        proj    = 20
#        popt = predict_loss_projection(monitor_steps, monitor_losses, 0, proj*1)
        popt  = LossForecaster.predict_loss_projection(steps_h, vals_h, 0, proj)
        preds = {}
        for h in horizons:
            preds[h] = LossForecaster.power_law(steps_h[-1] + h, *popt)

        stats          = LossForecaster.check_efficiency(popt, steps_h[-1], horizon)
        status_plateau = f"{UI['RED']}\u2715 Non" if stats["is_plateau"] else f"{UI['GREEN']}\u2714 Yes !"
        asymptote      = stats['loss_horizon']
        _, _, color_asympt = Analyzer.get_intel_level(asymptote)
        ...
        if preds:
            ...
            for h, val in preds.items():
                ...
                pad  = Config.LIGNE_LEN - 16 - (len(txt) - 8)
        ...
        for p1, p2, p3 in zip(params_[:5], params_[5:10], params_[10:]):
            ...
            if (params[p3] == False):
                txt3 = f"{UI['RED']}\u2718{r}"
            print(f"{txt}" + " " * (28 - len(txt)) + f"{txt2}" + " " * (25 - len(txt2)) + f"| {p3}: {txt3}")
        print(f"{UI['GRAY']}" + f"─" * Config.LIGNE_LEN + f"{UI['RESET']}")

        print(f"  🔄 Effective Batch Size (EBS) : {UI['BOLD']}{EBS}{UI['RESET']}            | ", end="")
        ema_model_status = f"{UI['GREEN']}\u2714" if self.checkpoint_loader.model_ema is not None else f"{UI['RED']}\u2718"
        print(f"  {UI['BOLD']}{ema_model_status}{UI['RESET']} Model EMA")
        print(f"  📥 Taille du Model : {UI['BOLD']}{self.checkpoint_loader.n_params/1e6:.1f}M{UI['RESET']} Paramètres")
        print(f"    ", end="")
        ...
        update_icon = f"{UI['MAGENTA']}{UI['BOLD']}🔊 {UI['CYAN']}" if self.update['status'] == 1 else ""
        ...
        self.update['elapse'] = d['elapse']
        pattern     = [' ', '⡀', '⡄', '⡆', '⡇']
        val         = round(elapse * 5)
        cpt         = f"{pattern[val%5]}" * 1
        loss_avg    = np.mean([d['loss'] for d in data_monitor[-100:]])  # *(1 - config['dropout'])
        loss_avg    = Utils.calcul_ema([v['loss'] for v in data_monitor])
        arrow       = f"{UI['B_YEL']}\u2b06{UI['CYAN']}" if (d['loss'] > loss_avg) else f"{UI['B_MAG']}\u2b07{UI['CYAN']}"
        print(f"{UI['CYAN']}   {star}STEP: {d['step']}    | SPD : {(d['elapse']/d['inter']):.2f}/st | LOSS train : {d['loss']:.3f} {arrow} | EMA : {loss_avg:.3f}{UI['RESET']}")
        # Grad Norm en live ...
        print(f"{UI['GRAY']}" + f"─" * Config.LIGNE_LEN + f"{UI['RESET']}")
        print(f"   {gn_status}{UI['CYAN']}  | Gradient Norm. variations \u2193\u2191  : [{gn_min:.3f} - {gn_max:.3f}]")

        # Step dynamique (incrémente en temps réel)
        pct          = min(p_step, 1.01) * 100.
        bar          = "▬" * (round(pct/4)-1) + f"{UI['RED']}\u2719" + UI['DIM'] + UI['WHITE'] + "┅" * (25 + (-1 if round(pct/4)==0 else 0) - round(pct/4))  # "▄"
#        bar          = "▬" * (int(pct/6)-1) + f"{UI['RED']}▬" + f"{UI['GRAY']}" + "┅" * (25 - int(pct/6))
        ...
        print(f"{UI['CYAN']}   {current_interval:3d} stp/run  {UI['RED'] if p_step > 0.95 else UI['CYAN']} ┣{bar}┫ {UI['RESET']}{UI['CYAN']} |{pct:4.1f}|  {next_run.strftime('%H:%M')} {run_ecart}){UI['RESET']}")
...
# =============================================================================
dashboard     = Dashboard()
last_time     = Dashboard.get_last_modified(Config.MONITOR_FILE)
last_time_log = Dashboard.get_last_modified(Config.HISTORY_FILE)
os.system('clear')
print(f"{UI['GRAY']}" + f"─" * Config.LIGNE_LEN + f"{UI['RESET']}")
Titre       = f"  🚀   M1 - LLM Training Monitor {Config.VERSION}."
print(f"\033[1m{Titre}\033[0m")
print(f"{UI['GRAY']}" + f"─" * Config.LIGNE_LEN + f"{UI['RESET']}")
print(f"📡 Monitoring réactif activé\n  - {Config.MONITOR_FILE}\n  - {Config.HISTORY_FILE}")
print(f"{UI['GRAY']}" + f"─" * Config.LIGNE_LEN + f"{UI['RESET']}")
timeout_pattern = ['⠋', '⠙', '⠹', '⠸', '⠼', '⠴', '⠦', '⠧', '⠇', '⠏']
timeout = 0
update_cpt = 0
while True:
    current_time_M       = Dashboard.get_last_modified(Config.MONITOR_FILE)
    current_time_L       = Dashboard.get_last_modified(Config.HISTORY_FILE)
    current_time         = max(current_time_M, current_time_L)
    seconds_since_update = time.time() - current_time
    progression          = seconds_since_update / max(1, dashboard.update['elapse'])
    ...
    if (update_cpt>200) or (update_cpt<15):
        print(f"{UI['MAGENTA']}   {timeout_pattern[timeout]}  Monitoring by eCoucou \u00A92026 : {UI['RED'] if progression>0.95 else UI['RESET']}{min(100, progression*100):5.1f}% {bar}      ", end="\r")
    else:
        print(f"{UI['BLUE']}   {timeout_pattern[timeout]}  Progression estimée du step : {UI['RED'] if progression>0.95 else UI['RESET']}{min(100, progression*100):5.1f}% {bar} \u00A92026 ", end="\r")
    # Si le fichier a été modifié depuis la dernière vérification
    if (current_time > last_time) | (dashboard.checkpoint_loader.first == True):
        try:
            time.sleep(6)
            print(f"{UI['ORANGE']}   ⏲ Updating data in progress ... " + " "*22 + "\u00A9 2026 e-Coucou ", end="\r")
    ...
    if (update_cpt>220):
        try:
            print(f"{UI['B_OR']}   {timeout_pattern[timeout]}  Monitoring by eCoucou \u00A92026 : {UI['RED'] if progression>0.95 else UI['RESET']}{min(100, progression*100):5.1f}% {bar}      ", end="\r")
            dashboard.get_dashboard()
            update_cpt=0
        except Exception as e:
            print(f'error: {e}')
