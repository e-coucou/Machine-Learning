...
#batch_size = 32 #32 # how many independent sequences will we process in parallel
...
#n_layers = 6 #6 # number of transformer blocks
#dropout = 0.2 # dropout rate
# data Loading ------------------------------------------------

class Head_v0(nn.Module):
...
class MultiHeadAttention_v0(nn.Module):
...
class FeedForward_v0(nn.Module):
...
class Transformer(nn.Module):
    ...
    def forward(self, source_idx, target_idx, targets=None):
        ...
        enc_output = self.encoder(source_idx,padding=src_mask) # (B, T_src, n_embd)
...
class BigramLanguageModeler_v0(nn.Module):
...
#        self.ffwd = FeedForward(n_embd) # feed-forward layer
    ...
    def forward(self, idx, targets=None, paddings=None):
        B, T = idx.shape
        ...
        x = self.blocks(x, padding=paddings)  # (B,T,C)
...
class Model_v0():
    def __init__(self, text_encode, token, fileout='data/test.pth'):
        ...
        print(self.device)
        if torch.backends.mps.is_available():
            torch.set_float32_matmul_precision('medium')  # ou 'medium' pour plus rapide
        ...
        # OPTION 1: Compiler TOUT le modèle (le plus simple)
        # Mode 'reduce-overhead': meilleur pour les petits modèles et M1
    ...
    def genere(self, prompt=None,new_token=1000):
        ...
        with torch.no_grad():
            if prompt is None:
                ...
                prompt_tokens = self.token.encode_np(prompt)  # Retourne une liste de tokens
